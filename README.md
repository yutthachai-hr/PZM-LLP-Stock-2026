...
| 5 | เก็บบน Cloud | ใช้ได้ทุกเครื่องแบบเรียลไทม์, เปิดดูออฟไลน์ได้ (บันทึกต้องมีเน็ต) |
...
### 1) โหมดในเครื่อง (ใช้เมื่อ **ยังไม่ได้** ใส่ค่า Firebase)

> ⚠️ repo นี้ฝังค่า Firebase ของโครงการจริงไว้ใน `src/firebase/config.ts` แล้ว
> **เปิดแอปขึ้นมาจะเป็นโหมด Cloud ต่อฐานข้อมูลจริงทันที** ไม่ใช่โหมดในเครื่อง
> ถ้าจะทดลองโดยไม่แตะข้อมูลจริง ให้ตั้ง `BUILT_IN` เป็น `null` ชั่วคราว
...
- **offline: อ่านได้ บันทึกไม่ได้** — PWA เก็บหน้าจอและข้อมูลที่โหลดไว้แล้ว จึงเปิดดูสต๊อกตอนเน็ตหลุดได้
...
- **สำรองข้อมูล:** ⚠️ **ไม่มีการสำรองอัตโนมัติ** Google เก็บข้อมูลไว้หลายศูนย์ข้อมูลจริง แต่นั่นกัน
  *เครื่องพัง* ไม่ได้กัน *ลบผิด* — แพ็กเกจฟรีไม่มี point-in-time recovery และไม่มี scheduled backup
  (ทั้งสองอย่างต้องเปิดบัญชีเรียกเก็บเงิน) **ไฟล์สำรองที่กดเองในหน้าตั้งค่าคือทางกู้คืนทางเดียวที่มี**
  ควรกดอย่างน้อยสัปดาห์ละครั้งแล้วเก็บไฟล์ไว้ใน Drive
