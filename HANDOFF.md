...
### แก้ไขโปรไฟล์ของตัวเอง + จัดคอลัมน์จำนวน/หน่วยให้ตรงกัน (22 ก.ย., branch `feat/edit-profile`, ต้อง deploy rules)
...
- **ต้อง deploy `firestore.rules` ก่อนเปลี่ยนชื่อตัวเองใช้ได้จริงบนของจริง** — เดิม `users/{uid}` ให้แอดมินแก้ได้เท่านั้น แม้จะแก้ชื่อตัวเอง เพิ่ม branch ใหม่: บัญชี active แก้ `name` ของตัวเองได้ (เท่านั้น — ไม่แตะ role/active/email) โดยไม่ต้องเป็นแอดมิน. เทสต์ `tests/firestore-rules.test.ts` (+2 คดี) ผ่านครบ 158 คดี. **การเปลี่ยนรหัสผ่านใช้ได้ทันทีไม่ต้อง deploy rules** (เป็น Firebase Auth ล้วน ไม่แตะ Firestore)
